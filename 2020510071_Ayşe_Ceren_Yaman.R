...
decision_tree <- function(X_train, y_train, k = 10, cp_values = seq(0.01, 0.1, 0.01)) {
  ...
  for (j in 1:k) {
    ...
    # Evaluate the model
    y_pred_cv <- as.factor(pred_val)
    y_test_factor <- as.factor(test_temp$isFraud)
    conf_matrix_cv <- confusionMatrix(y_pred_cv, y_test_factor)
    
    # Print evaluation metrics for each fold
    print(paste("Fold", j, "Confusion Matrix:"))
    print(conf_matrix_cv)
    print(paste("Fold", j, "F1 Score:", conf_matrix_cv$byClass["F1"]))
    print(paste("Fold", j, "Recall:", conf_matrix_cv$byClass["Recall"]))
    print(paste("Fold", j, "AUC:", conf_matrix_cv$byClass["Area under ROC"]))
    print(paste("Fold", j, "Precision:", conf_matrix_cv$byClass["Precision"]))
    
    out <- rbind(out, data.frame(predicted = pred_val, Fold = j))
  }
  
  # Display average evaluation metrics across all folds
  avg_metrics <- colMeans(sapply(1:k, function(i) {
    y_pred_fold <- as.factor(out[out$Fold == i, ]$predicted)
    y_test_fold <- as.factor(df[df$Fold == i, ]$isFraud)
    conf_matrix_fold <- confusionMatrix(y_pred_fold, y_test_fold)
    conf_matrix_fold$byClass
  }))
  ...
  # Display other average metrics
  print("Average F1 Score, Recall, AUC, and Precision (Cross-Validation):")
  print(avg_metrics)
  
  # Plot decision tree performance metrics
  plot_decision_tree(avg_metrics)
  
  print("########################################################")
}
plot_decision_tree <- function(avg_metrics) {
  # Ensure avg_metrics is a named numeric vector
  if (!is.numeric(avg_metrics) || is.null(names(avg_metrics)) || length(avg_metrics) == 0) {
    stop("avg_metrics must be a non-empty named numeric vector.")
  }
  
  # Prepare data for plotting
  metrics_names <- names(avg_metrics)
  metrics_values <- as.vector(avg_metrics)
  
  # Create a data frame for plotting
  plot_data <- data.frame(Metric = as.factor(metrics_names), Value = metrics_values)
...
logistic_regression <- function(X, y, folds = 5, lambda_values = seq(0.01, 1, 0.1)) {
  ...
  for (fold_num in seq_along(folds_list)) {
    ...
    # Store metrics
    precision <- c(precision, conf_matrix$byClass["Pos Pred Value"])
    recall <- c(recall, conf_matrix$byClass["Sensitivity"])
...
svm <- function() {
  ...
  for (fold in 1:num_folds) {
    ...
    # Calculate and print precision, recall, F1 score, and kappa
    precision <- svm_results[[fold]][2, 2] / sum(svm_results[[fold]][, 2])
    recall <- svm_results[[fold]][2, 2] / sum(svm_results[[fold]][2, ])
    f1_score <- 2 * precision * recall / (precision + recall)
    
    # Calculate kappa
    observed_agreement <- sum(diag(svm_results[[fold]])) / sum(svm_results[[fold]])
    chance_agreement <- sum(rowSums(svm_results[[fold]]) * colSums(svm_results[[fold]])) / sum(svm_results[[fold]])^2
    kappa <- (observed_agreement - chance_agreement) / (1 - chance_agreement)
    
    roc_obj <- roc(svm_results[[fold]][, 2], svm_results[[fold]][, 1])
  ...
  plot_svm(svm_results,auc_value)
  print("########################################################")
}

plot_svm <- function(results, auc_values) {
  ...
  # Prepare data for plotting
  data <- data.frame(
    Metric = rep(c("Precision", "Recall", "F1 Score", "AUC"), each = length(results)),
    Fold = rep(seq_along(results), times = 4),
    Value = c(precision, recall, f1_score, rep(auc_values, each = length(results)))
  )
